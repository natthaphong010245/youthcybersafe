<div class="bg-[#929AFF] rounded-xl p-3 relative flex items-center">
    <div class="min-w-[80px] h-[80px] bg-white rounded-full flex items-center justify-center overflow-hidden mr-4">
        <img src="{{ asset('images/report_consultation/country/' . $image) }}" 
             alt="{{ $name }}" 
             class="{{ $image_size }} object-contain">
    </div>
    
    <div class="flex-grow">
        <div class="text-white font-medium text-lg leading-tight">{{ $name }}</div>
        
        @if($phone)
        <a href="tel:{{ $phone }}" class="flex items-center text-white text-lg">
